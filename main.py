...
calibrate = False
x_default, y_default = 0, 0
global jump
jump = False

# Define function to check for jump and duck
def check_jump(y):
    if y < y_default - 50:
        return True
    else:
...
def cam():
    global calibrate, x_default, y_default, jump
    ...
    while True:
        ...
        # Loop over each face and draw bounding box
        ...
        for output in layerOutputs:
            for detection in output:
                ...
                if confidence > 0.5:
                    if not calibrate and len(faces) != 0:
                        x_default, y_default = faces[0][0], faces[0][1]
                        if x_default > 0 and y_default > 0:
                            calibrate = True
                    # Object detected (person)
                    if x_default > 0 and y_default > 0 and check_jump(y):     
                        jump = True  
                        print(jump)
                    elif not check_jump(y):
                        jump = False

                    if x_default > 0 and y_default > 0 and check_duck(y):     
                        print("Duck")       
...
opencv_thread = threading.Thread(target=cam)
opencv_thread.start()

# Continue with the Pygame loop
pygame.init()
screen = pygame.display.set_mode((1280, 720))
clock = pygame.time.Clock()
pygame.display.set_caption("Dino Game")

# Your existing Pygame code...
